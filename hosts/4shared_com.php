	...
	if(empty($cookie)==false || ($user && $pass)){
		for ($j=0; $j < 2; $j++){
			...
			if(!$cookie){
				...
				$data = $this->curl("http://www.4shared.com/web/login","4langcookie=en","login=$user&password=$pass&remember=on&doNotRedirect=true");
			...
			$this->cookie = $cookie;	
			//die('<textarea>'.$data . '</textarea>');
			if (stristr($data,'The file link that you requested is not valid')) die(Tools_get::report($Original,"dead"));
			elseif(strpos($data,'TYPE="PASSWORD"')) die(_reportpass);
			elseif(preg_match('/id="btnLink" href="(.*)" class="gaClick/i', $data, $linkpre)) {
				$link = trim ($linkpre[1]);
